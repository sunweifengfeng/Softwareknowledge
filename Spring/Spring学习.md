...
11) 如果在` <bean> `中指定了该 Bean 的作用域为 prototype，则将该 Bean 交给调用者，调用者管理该 Bean 的生命周期，Spring 不再管理该 Bean。
...
```
您可以实现以上接口，在 destroy 方法内指定 Bean 销毁前需要执行的操作。
```java
<bean id="..." class="..." />
public class User implements InitializingBean {
    @Override
...
### 示例
下面使用Idea演示如何通过配置 XML 的方式实现初始化回调和销毁回调，步骤如下：
1. 创建 SpringDemo 项目，并在 java 目录下创建 net.biancheng 包。
2. 添加相应的maven jar 包，可以参考《第一个Spring程序》一节。
3. 在 net.biancheng 包下创建 HelloWorld 和 MainApp 类。
4. 在 resource目录下创建 Spring 配置文件 Beans.xml。
5 运行项目。
...
## 九 BeanPostProcessor（Spring后置处理器）
